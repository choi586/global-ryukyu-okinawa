...
`pnpm migrate:supabase --dry-run`으로 이전 대상과 파일의 존재를 확인합니다. 실제 연결 이후 서버 전용 환경변수를 설정하고 `pnpm migrate:supabase`를 실행하면 기존 게시일·주소를 유지하여 옮깁니다. 관리자의 로그인 세션이나 로컬 로그인 정보는 이전하지 않습니다. 원격에 내용이 다른 동일 ID의 글이 있으면 덮어쓰지 않고 중단합니다. 원본 데이터는 보존합니다.
